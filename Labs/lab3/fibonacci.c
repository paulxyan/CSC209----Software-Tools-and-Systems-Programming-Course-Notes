...
void fib(int **fib_sequence1, int count) {
    // Allocate memory for the Fibonacci sequence
    *fib_sequence1 = (int *)malloc(sizeof(int) * count);
    if (*fib_sequence1 == NULL) { // Check if malloc succeeded
    ...
    for (int j = 0; j < count; j++) {
        if (j == 0) {
            (*fib_sequence1)[j] = 0; // First element
        } else if (j == 1) {
            (*fib_sequence1)[j] = 1; // Second element
        } else {
            (*fib_sequence1)[j] = (*fib_sequence1)[j - 1] + (*fib_sequence1)[j - 2];
        }
    }
}
