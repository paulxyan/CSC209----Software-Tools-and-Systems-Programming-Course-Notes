...
int read_from(int client_index, struct sockname *users) {
    ...
    char *new_str = malloc((sizeof(char) * strlen(users[client_index].username)) + 2);

    strncpy(new_str, users[client_index].username, strlen(users[client_index].username) + 2);

    new_str[strlen(users[client_index].username)-1] = ':';
    new_str[strlen(users[client_index].username)] = ' ';
    new_str[strlen(users[client_index].username)+1] = '\0';
    ...
    while (user_index < MAX_CONNECTIONS) {
        if (users[user_index].sock_fd != -1) {
            ...
            if (empty) {
                write(users[user_index].sock_fd, new_str, strlen(new_str));
            
        
            if (write(users[user_index].sock_fd, buf, strlen(buf)) != strlen(buf)) {
                users[client_index].sock_fd = -1;
                return fd;
            }
        }
